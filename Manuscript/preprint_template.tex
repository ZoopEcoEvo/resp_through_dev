...
%  \begin{center}%
  \let \footnote \thanks
    {\fontsize{18}{20}\selectfont\raggedright  \setlength{\parindent}{0pt} \@title}%
}
%\fi
\makeatother
...
$if(title)$
\title{\textbf{\LARGE $title$}$if(subtitle)$: $subtitle$$endif$ $if(anonymous)$ $endif$ }
$endif$ 


$if(anonymous)$$else$\author{$for(author)$\Large $author.name$\vspace{0.01in} \newline\normalsize\emph{$author.affiliation$}  $sep$ \and $endfor$}$endif$
...
\titlespacing*{\section}
  {0pt}{1\baselineskip}{0.25\baselineskip}
\titlespacing*{\subsection}
  {0pt}{1\baselineskip}{0.25\baselineskip}

\usepackage[square,sort,comma,numbers]{natbib} % use \usepackage[square,sort,comma,numbers]{natbib}  if numeric in-text citations are desired
...
$if(anonymous)$\hfill $else$$for(author)$\textbf{\authorfont $author.name$} \hskip 5pt \emph{\small $author.affiliation$}  $sep$ \newline $endfor$ $endif$
...
\vskip 5pt \noindent \small \emph{Keywords}: $keywords$ \par
